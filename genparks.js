...
const sourceParkDataArr = [
  ...
  description: [
    'The block with the number one million was mined on February 13 2016, nearly 200 days after the mainnet came to life. This period amounts to roughly 17.1 million seconds, or approximately one new block every 17 seconds on average. This metric, known as the average block time, has varied widely over the years, sometimes being significantly higher than the 15 seconds target. A key influencer of block time is the difficulty bomb. This mechanism increases the difficulty exponentially after some time, forcing the network and the community to upgrade to a new version through a hard fork to reset the bomb.'
  ]
},  
  ...
  description: [
    'The ‘Homestead’ update, released on Pi Day (March 14th, 2016) at block 1,150,000, is widely considered the first “stable” Ethereum release. Initially planned to deploy a month after Frontier, it actually took the developers 7.5 months to complete the extensive project.',
    '',
    'Ethereum forks include a number of Ethereum Improvement Proposals (EIP). Homestead implemented 3 EIPs (EIP-2, 7 and 8). These EIPs cover a broad number of improvements, including an increase to the cost of contract creation to address a protocol bug which allowed value transfer at 11664 gas, the introduction of the DELEGATECALL opcode which enabled safer smart contract design pattern such as proxy contracts, and changes to the difficulty calculation, in hope of restoring an average block time of 15 seconds (which had recently averaged about 1 block per 13 seconds).'
  ]
}, 
  ...
  description: [
    'While theDAO hack was probably mostly financially motivated, the EXTCODESIZE DoS attack had no obvious financial motives (apart from possibly gains from shorting ether, although this would not have paid off significantly since price only shortly dropped before rallying a few dollars above). So why would anyone spend time identifying and exploiting this vulnerability in geth?',
    '',
  ...
  description: [
    'The DoS attacks of Fall 2016 took a serious toll on the immediate network health of the Ethereum blockchain; with a large number of nodes not being able to sync, hard forks had to be quickly planned and implemented by client teams in order to restore operations. The first fork, named Tangerine Whistle, fixed several underpriced IO-heavy operations, as specified in EIP-150. This non-contentious fork occurred on October 18th 2016, approximately one month after the first DoS attack took place.'
  ]
}, 
  ...
  description: [
    'The last Pi block was mined on February 7th 2017, with a total of 7 digits, it occurred almost 1.5 years after the previous Pi block of 6 digits (#314,159). Assuming an average block time of 10 seconds, the next Pi block (#31,415,926) should happen somewhere around 2026.'
  ]
},
  ...
  description: [
    'Decentraland, the Virtual Reality (VR) world that you are currently visiting, had a first working version called the Bronze Age. From the whitepaper:',
    '',
  ...
  description: [
    'Then came the Terraform, the initial generation of LAND within Decentraland in September 2017. It was handled by terraform.aetheriablockmuseum.eth, created at block 4,321,454. Initially the cutoff date was December 15, 2018 but in the end people were able to lock more MANA into the Terraform contract while the auction was live, thus creating 70,399 parcels of LAND. A whopping 161,483,050 MANA (roughly $10m) was spent in what became the largest ever virtual land sale.'
  ]
},    
  ...
  description: [
    'On October 16, 2017, Metropolis - an Ethereum development roadmap - began the first of its two stages with the Byzantium fork. Activated at block 4,370,000, Byzantium ushered in major improvements to the Ethereum blockchain including the introduction of STATICCALL (EIP-214); a “status” field in transaction receipts for easy verification by light clients (EIP-658) and a fix to the uncle rewards (EIP-100). Other significant changes with Byzantium are the addition of new opcodes and precompiled contracts to make it easier for developers to experiment with zkSNARKs (zkSNARKs enable clients to provide proof to a verifier that they possess a secret without revealing said secret to the verifier).',
    '',
  ...
  description: [
    'Between October 30th and November 2nd, 2018, Devcon-4 took place in Prague. With 3,000 attendees, Devcon-4 was the largest crypto convention to date. Major topics involved: scalability, security, privacy, developer experience, society and design. Updates were included from:  from the Swarm team, the STARKs workshop, optimization of smart contracts, the Raiden Network and the state of Plasma. Perhaps the most vital convention yet, Devcon-4 focused on scaling the platform so that it would be ready for mainstream adoption.'
  ]
},
]
