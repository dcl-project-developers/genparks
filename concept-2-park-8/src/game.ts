...
function buildCube(blockNumber, cubeNumber, cubeHexDigit) {
  ...
  do {
    ...
    if(cubeDecimal === mod) {
      // decorator color
      let decoratorColorHex = '#' + (index + 6) + '0' + (index + 4) + '0' + (index + 3) + '0'
      ...
      let decoratorCubeWidth = width/8.0 * (index + 1)
      let decoratorCubeLength = length/8.0 * (index + 1)
      let decoratorCubeHeight = height/8.0 * (index + 1)
    ...
    index += 1
    console.log('===== for index ', index, 'mod', mod, 'result', result, 'value', value)
  } while(value > 0)
...
buildArtwork(2, 1428757, '17fea357e1a1a514b45d45db586c272a7415f8eb8aeb4aa1dcaf87e56f34ca59')
